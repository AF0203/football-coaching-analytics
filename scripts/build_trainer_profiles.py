...
REFERENCE_FILE = PROCESSED_DIR / "bl_reference.csv"
TRAINER_INDEX  = TRAINER_DIR / "trainer_index.csv"

# ── Bundesliga-Referenz laden ─────────────────────────────────────
bl_ref      = pd.read_csv(REFERENCE_FILE)
bl_ref_dict = {
...
def process_trainer_raw(raw_file, trainer_id):
    ...
    liga_id = df["liga_id"].iloc[0] \
        if "liga_id" in df.columns else "bundesliga"
    ...
    if n_trainer == 0:
        raise ValueError(
            f"Keine Trainer-Events! teamIds: {df_clean['teamId'].unique()}"
        )
...
def save_trainer_profile(df, trainer_id, name):
    """Speichert Profil in trainer_profiles/profiles_{name}.csv"""
...
def build_cluster_profile_trainer(seq_df, km, K,
    ...
    Identisch zu build_profiles.py aber gegen BL-Referenz
    """
...
def build_abstoß_profile(actions, df_all, trainer_id, n_games):
    ...
    profile = build_cluster_profile_trainer(
        seq_res, km, K, "abstoß", trainer_id, n_games
    )
...
def build_aufbau_profile(actions, df_all, trainer_id, n_games):
    ...
    seq_res = pd.DataFrame({"sequence_id":sids,"teamId":tids,"cluster":labels})
    profile = build_cluster_profile_trainer(
        seq_res, km, K, "aufbau", trainer_id, n_games
    )
...
def build_mf_profile(actions, df_all, trainer_id, n_games):
    ...
    seq_res = pd.DataFrame({"sequence_id":sids,"teamId":tids,"cluster":labels})
    profile = build_cluster_profile_trainer(
        seq_res, km, K, "mf", trainer_id, n_games
    )
...
def build_prog_profile(actions, df_all, trainer_id, n_games):
    ...
    seq_res = pd.DataFrame({"sequence_id":sids,"teamId":tids,"cluster":labels})
    profile = build_cluster_profile_trainer(
        seq_res, km, K, "prog", trainer_id, n_games
    )
...
def build_shot_profile(actions, df_all, trainer_id, n_games):
    ...
    seq_res = pd.DataFrame({"sequence_id":sids,"teamId":tids,"cluster":labels})
    profile = build_cluster_profile_trainer(
        seq_res, km, K, "shot", trainer_id, n_games
    )
...
def build_transition_profile(actions, df_all, trainer_id, n_games):
    ...
    seq_res = pd.DataFrame({"sequence_id":sids,"teamId":tids,"cluster":labels})
    profile = build_cluster_profile_trainer(
        seq_res, km, K, "transition", trainer_id, n_games
    )
...
def build_gegenpressing_profile(actions, df_all, trainer_id, n_games):
    ...
    profile = build_cluster_profile_trainer(
        ...
        "gegenpressing", trainer_id, n_games
    )
    avg_recovery = press_df[
        press_df["teamId"]==trainer_id
    ]["time_to_recovery"].mean()
    profile["avg_time_to_recovery"] = avg_recovery
    profile["recovery_speed_zscore"] = get_zscore(
        "gegenpressing","avg_time_to_recovery",avg_recovery
    )
...
def build_high_block_profile(actions, df_all, trainer_id, n_games):
    ...
    for m_id, m_df in df_all.groupby("matchId"):
        ...
        for _, cand in candidates.iterrows():
            ...
            if next_row.empty:
                continue
            if next_row["team_id"].iloc[0]!=trainer_id:
                continue
...
def build_mid_low_block_profile(actions, df_all, trainer_id, n_games):
    ...
    profile = build_cluster_profile_trainer(
        ...
        "mid_low_block", trainer_id, n_games
    )
...
def build_prog_against_profile(actions, df_all, trainer_id, n_games):
    ...
    seq_res = pd.DataFrame({
        "sequence_id":sids,
        "teamId":[trainer_id]*len(sids),
        "cluster":labels
    })
    profile = build_cluster_profile_trainer(
        seq_res, km, K, "prog_against", trainer_id, n_games
    )
...
def build_shots_against_profile(actions, df_all, trainer_id, n_games):
    ...
    opp_id    = f"opp_{trainer_id}"
    shots_opp = df_all[
        (df_all["type"]=="Shot") & (df_all["teamId"]==opp_id)
    ].copy()
    ...
    profile = build_cluster_profile_trainer(
        seq_res, km, K, "shots_against", trainer_id, n_games
    )
...
def build_aufbau_abbruch_profile(actions, df_all, trainer_id, n_games):
    ...
    seq_res = pd.DataFrame({"sequence_id":sids,"teamId":tids,"cluster":labels})
    profile = build_cluster_profile_trainer(
        seq_res, km, K, "aufbau_abbruch", trainer_id, n_games
    )
...
def build_pass_score_profile(actions, df_all, trainer_id):
    try:
        ...
        passes = df_all[
            (df_all["type"]=="Pass") &
            (df_all["teamId"]==trainer_id) &
            df_all["x"].notna() & df_all["endX"].notna()
        ].copy()
...
def build_network_profile(df_all, trainer_id):
    ...
    net_df = pd.DataFrame([{"teamId":trainer_id,"liga":"bundesliga",**net_features}])
...
def build_performance_profile(actions, df_all, trainer_id):
    ...
    for match_id, m_df in df_all.groupby("matchId"):
        opp_pass = m_df[
            (m_df["teamId"]==opp_id) & (m_df["type"]=="Pass") & (m_df["x"]>50)
        ]
        own_def  = m_df[
            (m_df["teamId"]==trainer_id) &
            (m_df["type"].isin(DEFENSIVE_ACTIONS)) & (m_df["x"]>50)
        ]
        if len(own_def)==0:
            continue
    ...
    # Block Quality
    seq_opp_deep = df_all[
        (df_all["type"].isin(["Pass","Carry"])) & (df_all["teamId"]==opp_id)
    ...
    crossing_66_seqs = set(df_all[
        (df_all["type"].isin(["Pass","Carry"])) &
        (df_all["teamId"]==opp_id) &
        (df_all["outcomeType"]=="Successful") &
        (df_all["x"]<66) & (df_all["endX"]>=66)
    ]["sequence_id"].unique())

    opp_cross_50 = set(df_all[
        (df_all["type"].isin(["Pass","Carry"])) &
        (df_all["teamId"]==opp_id) &
        (df_all["outcomeType"]=="Successful") &
        (df_all["x"]<50) & (df_all["endX"]>=50)
    ]["sequence_id"].unique())
    ...
    perf_vector = np.nan_to_num(np.array([[
        perf[f"{c}_zscore"].iloc[0] if f"{c}_zscore" in perf.columns else 0
...
def build_profiles_for_trainer(trainer_id, trainer_name,
                                verein, liga_id, raw_file):
    print(f"\n{'─'*60}")
    print(f"  {trainer_name} ({trainer_id})")
    print(f"{'─'*60}")
    ...
    df_clean["sequence_id"] = (
        trainer_id + "_" + df_clean["sequence_id"].astype(str)
    )

    n_games = df_clean[
        df_clean["teamId"]==trainer_id
    ]["matchId"].nunique()

    print(f"  Events:    {len(df_clean):,}")
    print(f"  Spiele:    {n_games}")
    print(f"  Sequenzen: {df_clean['sequence_id'].nunique():,}")

    actions = df_clean[
        (df_clean["type"].isin(["Pass","Carry"])) &
        (df_clean["teamId"]==trainer_id)
    ].copy()
    ...
    try:
        print("    → Abstoß...")
        build_abstoß_profile(actions, df_clean, trainer_id, n_games)
    except Exception as e:
        print(f"      ⚠️  {e}")

    try:
        print("    → Aufbau...")
        build_aufbau_profile(actions, df_clean, trainer_id, n_games)
    except Exception as e:
        print(f"      ⚠️  {e}")

    try:
        print("    → Mittelfeld...")
        build_mf_profile(actions, df_clean, trainer_id, n_games)
    except Exception as e:
        print(f"      ⚠️  {e}")

    try:
        print("    → Progression...")
        build_prog_profile(actions, df_clean, trainer_id, n_games)
    except Exception as e:
        print(f"      ⚠️  {e}")

    try:
        print("    → Vor Schuss...")
        build_shot_profile(actions, df_clean, trainer_id, n_games)
    except Exception as e:
        print(f"      ⚠️  {e}")

    try:
        print("    → Transition...")
        build_transition_profile(actions, df_clean, trainer_id, n_games)
    except Exception as e:
        print(f"      ⚠️  {e}")

    try:
        print("    → Gegenpressing...")
        build_gegenpressing_profile(actions, df_clean, trainer_id, n_games)
    except Exception as e:
        print(f"      ⚠️  {e}")

    try:
        print("    → High Block...")
        build_high_block_profile(actions, df_clean, trainer_id, n_games)
    except Exception as e:
        print(f"      ⚠️  {e}")

    try:
        print("    → Mid/Low Block...")
        build_mid_low_block_profile(actions, df_clean, trainer_id, n_games)
    except Exception as e:
        print(f"      ⚠️  {e}")

    try:
        print("    → Progression gegen sich...")
        build_prog_against_profile(actions, df_clean, trainer_id, n_games)
    except Exception as e:
        print(f"      ⚠️  {e}")

    try:
        print("    → Schüsse gegen sich...")
        build_shots_against_profile(actions, df_clean, trainer_id, n_games)
    except Exception as e:
        print(f"      ⚠️  {e}")

    try:
        print("    → Aufbau-Abbrüche...")
        build_aufbau_abbruch_profile(actions, df_clean, trainer_id, n_games)
    except Exception as e:
        print(f"      ⚠️  {e}")

    try:
        print("    → Pass-Score...")
        build_pass_score_profile(actions, df_clean, trainer_id)
    except Exception as e:
        print(f"      ⚠️  {e}")

    try:
        print("    → Formation...")
        build_formation_profile(df_clean, trainer_id)
    except Exception as e:
        print(f"      ⚠️  {e}")

    try:
        print("    → Netzwerk...")
        build_network_profile(df_clean, trainer_id)
    except Exception as e:
        print(f"      ⚠️  {e}")

    try:
        print("    → Performance...")
        build_performance_profile(actions, df_clean, trainer_id)
    except Exception as e:
        print(f"      ⚠️  {e}")

    try:
        print("    → Meta-Cluster...")
        build_meta_cluster_trainer(trainer_id)
    except Exception as e:
        print(f"      ⚠️  {e}")
...
def build_all_trainer_profiles(trainer_filter=None):
    ...
    if trainer_filter:
        trainer_index = trainer_index[
            trainer_index["trainer_id"]==trainer_filter
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Einzelnen Trainer verarbeiten z.B. 'tr_001'"
    )
